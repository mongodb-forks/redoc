...
// @ts-ignore
const redoc_1 = require('@dop/redoc');
...
const BUNDLES_DIR = path_1.dirname(require.resolve('@dop/redoc'));
...
const handlerForBuildCommand = argv =>
...
function serve(host, port, pathToSpec, options = {}) {
  return __awaiter(this, void 0, void 0, function* () {
    let spec = yield redoc_1.loadAndBundleSpec(
      isURL(pathToSpec) ? pathToSpec : path_1.resolve(pathToSpec),
    );
    ...
    const server = http_1.createServer((request, response) => {
      ...
      if (request.url === '/redoc.standalone.js') {
        respondWithGzip(
          fs_1.createReadStream(path_1.join(BUNDLES_DIR, 'redoc.standalone.js'), 'utf8'),
          request,
          response,
          {
...
function bundle(pathToSpec, options = {}) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const spec = yield redoc_1.loadAndBundleSpec(
      isURL(pathToSpec) ? pathToSpec : path_1.resolve(pathToSpec),
    );
    const pageHTML = yield getPageHTML(
      spec,
      pathToSpec,
      Object.assign(Object.assign({}, options), { ssr: true }),
    );
...
function getPageHTML(
  spec,
  pathToSpec,
  { ssr, cdn, title, disableGoogleFont, templateFileName, templateOptions, redocOptions = {} },
) {
  return __awaiter(this, void 0, void 0, function* () {
    let html;
    let css;
...
function respondWithGzip(contents, request, response, headers = {}) {
  ...
  if (acceptEncoding.match(/\bdeflate\b/)) {
    response.writeHead(
      200,
      Object.assign(Object.assign({}, headers), { 'content-encoding': 'deflate' }),
    );
    compressedStream = zlib.createDeflate();
  } else if (acceptEncoding.match(/\bgzip\b/)) {
    response.writeHead(
      200,
      Object.assign(Object.assign({}, headers), { 'content-encoding': 'gzip' }),
    );
